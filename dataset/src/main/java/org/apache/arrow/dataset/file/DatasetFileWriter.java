...
public class DatasetFileWriter {
   ...
   * @param baseNameTemplate file name template used to make partitions. E.g. "dat_{i}", i is
   *     current partition ID around all written files.
   * @param writerOptions format-specific writer options as key-value pairs. For Parquet, supported
   *     keys: "compression" (one of UNCOMPRESSED, SNAPPY, GZIP, ZSTD, LZ4, BROTLI,
   *     case-insensitive). May be null or empty to use defaults.
   */
  public static void write(
      BufferAllocator allocator,
      ArrowReader reader,
